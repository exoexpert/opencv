...
class Builder:
    ...
    def shouldEmbedBitcode(self, target):
        return not self.bitcodedisabled and target == 'iPhoneOS' and get_xcode_major() < 15

    def getCMakeArgs(self, arch, target):
        c_flags = APPLE_CLANG_FLAGS
        cxx_flags = APPLE_CLANG_FLAGS
        if self.shouldEmbedBitcode(target):
    ...
    def getBuildCommand(self, arch, target):
        ...
        if self.dynamic and self.shouldEmbedBitcode(target):
